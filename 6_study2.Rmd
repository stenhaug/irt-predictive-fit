...
# new but seems worse so i am going back
# read_rds("sims/sim2.rds") %>%
...
# 	mutate(var = factor(var, levels = c("ELPL-MR", "ELPL-MP", "BIC", "AIC", "LRT"))) %>% 
# 	mutate(`Winning model` = paste0(val, "PL")) %>%
# 	filter(`Winning model` != "1PL") %>% 
# 	rename(`N persons` = n_students, `Mean ability` = theta_mu) %>% 
# 	filter(var %in% c("ELPL-MR", "ELPL-MP")) %>% 
# 	mutate(x = case_when(var == "ELPL-MR" ~ "Missing Responses", var == "ELPL-MP" ~ "Missing Persons")) %>% 
# 	ggplot(aes(x = `Mean ability`, y = `N persons`, color = `Winning model`)) +
# 	geom_point(alpha = 0.5, size = 2.25) +
# 	facet_wrap(~ x, nrow = 1) +
# 	theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
# 	labs(
# 		x = latex2exp::TeX("Test Easiness"),
# 		y = latex2exp::TeX("People")
# 	) + 
